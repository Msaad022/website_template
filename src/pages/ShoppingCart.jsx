...
const ShoppingCart = () => {
  ...
  const updateCart = async () => {
    try {
      const res = await fetch(
        `${fitmasterHost}/cartPagination/${currentPage}?page=${currentPage}`
      );
      if (!res.ok) {
  ...
  const paginationdHandler = async (pageIndex) => {
    ...
    let page = pageIndex;
    let url = `${fitmasterHost}/cartPagination/${pageIndex}`
    try {
        setCurrentPage(page);
...
export const CartLoader = async () => {
  const res = await fetch(`${fitmasterHost}/cart/`);
  if (!res.ok) {
    throw Error("Could not find that Carts");
  }
